...
class GenerationMixin:
    ...
    def _validate_model_kwargs(self, model_kwargs: Dict[str, Any]):
        ...
        model_args.add('qflayer')
        model_args.add('qffolder')
        model_args.add('sys_len')
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            if do_sample:
                ...
                # if position_id==0:
                #     next_tokens = torch.tensor([151644], device='cuda:0')  # <|im_start|>
                # elif position_id==1:
                #     next_tokens = torch.tensor([77091], device='cuda:0')   # assistant
                # elif position_id==2:
                #     next_tokens = torch.tensor([198], device='cuda:0')   # \n
                ...
                if model_kwargs.get('qfmode') == 'QF-instruct' or model_kwargs.get('qfmode') == 'QF-update'  :
                    ...
                    try:
                        next_tokens = torch.tensor([token_id], device='cuda:0')
                    except:
                        next_tokens = torch.tensor([151645], device='cuda:0')
        ...
        if model_kwargs.get('qfmode') == 'QF-instruct':
            v=input_ids
            save_dir = '/home/ls/Github/ChatGLM3-main/QF/parameters'
            os.makedirs(save_dir, exist_ok=True)
            v_path = os.path.join(save_dir, f'layer{model_kwargs.get("qflayer")}_gt.pt')
            # 追加v
            v_data = v.detach().cpu().to(torch.float32)
            if os.path.exists(v_path):
                v_all = torch.load(v_path)
                v_all = torch.cat([v_all, v_data], dim=1)  # [1, n, 8960]
            else:
                v_all = v_data
